#!/usr/bin/env python3
"""
Optimized PGN Processor for Compact JSON Output (v2.2)
...
Updated to include all centrality metrics from directional_metrics_updated.py:
- Six centrality types: in/out degree, in/out betweenness, in/out closeness
- Proper statistical pooling for aggregated metrics
- Component-level averages and variances for all centralities
- Node-level deviations from component means
...
def flatten_move(rec):
    ...
    for color in ('combined','white','black'):
        ...
        # Map the full centrality names to shorter keys for compact JSON
        agg_compact = {
            'fiedler_value': agg.get('fiedler_value'),
            'out_diameter': agg.get('out_diameter'),
            'in_diameter': agg.get('in_diameter'),
            'in_degree_avg': agg.get('in_degree_centrality_avg'),
            'in_degree_var': agg.get('in_degree_centrality_var'),
            'out_degree_avg': agg.get('out_degree_centrality_avg'),
            'out_degree_var': agg.get('out_degree_centrality_var'),
            'in_betweenness_avg': agg.get('in_betweenness_centrality_avg'),
            'in_betweenness_var': agg.get('in_betweenness_centrality_var'),
            'out_betweenness_avg': agg.get('out_betweenness_centrality_avg'),
            'out_betweenness_var': agg.get('out_betweenness_centrality_var'),
            'in_closeness_avg': agg.get('in_closeness_centrality_avg'),
            'in_closeness_var': agg.get('in_closeness_centrality_var'),
            'out_closeness_avg': agg.get('out_closeness_centrality_avg'),
            'out_closeness_var': agg.get('out_closeness_centrality_var'),
            'modularity': agg.get('modularity'),
            'community_count': agg.get('community_count'),
            'clustering': agg.get('clustering'),
            'size_entropy': agg.get('size_entropy')
        }
        
        cmp=dm.get('component_level_metrics',[])
        gi=convert_graph_info_to_array(dm['graph_info'])
        graphs[color]={'agg':agg_compact,'cmp':cmp,'nds':gi['nodes'],'lks':gi['edges']}
...
def process_pgn(pgn_path, out_path):
    ...
    output={
        'metadata':{'schema_version':'2.2','description':'Compact chess game data for D3.js dashboards with all centrality metrics'},
...
if __name__=='__main__':
    ...
    output_json_path = "dubov_games.json"
